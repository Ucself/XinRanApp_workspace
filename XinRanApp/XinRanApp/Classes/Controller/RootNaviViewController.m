...
- (void)viewDidLoad {
    ...
    //[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(httpRequestUserLoginDone:) name:KNotification_UserLoginDone object:nil];
...
#pragma mark ---
//执行获取用户收获地址信息数据
-(void) httpRequestUserLoginDone:(NSNotification *)notification{
    [[NetInterfaceManager sharedInstance] getConsignee];
}
    ...
    switch (result.requestType) {
        case KReqestType_Login:
            if (result.resultCode == 0) {
                //登录成功通知去获取用户收获地址信息
                [self performSelectorOnMainThread:@selector(httpRequestUserLoginDone:) withObject:nil waitUntilDone:NO];
            }
            break;
        ...
        case KReqestType_Getconsignee:
        {
            ...
            {
                //缓存数据
                [EnvPreferences sharedInstance].userShipAddress = result.data;
            }
            else {
                [EnvPreferences sharedInstance].userShipAddress = nil;
...
//                }
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"updatebrandinfo" object:nil];
